<template>
    <div class="searchTop">
        ...
        <input type="text" placeholder="杨千嬅" v-model="searchKey" @keydown.enter="enterKey">
    </div>
...
export default {
    ...
    methods:{
        async enterKey(){
            if(this.searchKey!==""){
            this.keyWorldList.unshift(this.searchKey); //添加到数组最前
            // 去重
...
.searchHistory{
    ...
    .spanKey{
        padding: .1rem .2rem;
        background-color: rgb(195, 228, 189);
        border-radius: .4rem;
        margin: .1rem .2rem;
